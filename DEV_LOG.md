...
- ✅ **v0.2.28 报告可信度分层** - 历史列表和报告抬头将“已完成”“七项分析/三项速览”“数据齐全/部分数据获取成功/关键数据缺失”分开显示，并增加 0—5 星可信度；门控新增应采工具覆盖检查，完全未调用不再被误判为数据齐全，正常空结果仍算可用。
- ✅ **v0.2.30 确定性数据快照** - 数据采集从 LLM 手中移到代码层：七项分析固定 17 类/24 个请求，三项速览固定 10 类/17 个请求；生成带编号、时间戳、耗时和逐项状态的持久化快照，七位分析师只读各自子集且不绑定工具。最终结论增加代码一致性校验，评级缺失、评级与操作矛盾或关键数据缺失时不展示操作结论、不写入经验记忆。
- ⚠️ **v0.2.30 隔离 E2E 未通过** - 服务器临时容器运行 600879（2026-07-17），采集阶段记录行业对比/主力资金超时并继续，但 DeepSeek 决策链约 23 分钟仍未返回最终状态，人工停止；生产 v0.2.27 未改动。发布前必须补模型调用超时/阶段定位并复跑真实案例。
