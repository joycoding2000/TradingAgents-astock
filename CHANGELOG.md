...
- **全局 YAML 配置系统**：新增 `config.yaml.example` 模板，支持以“环境变量 > config.yaml > 默认值”的优先级调整分析参数。

### 改进

- **去除入口硬编码**：Web、CLI 和脚本统一使用 `get_config()` 读取配置。
...
- **确定性数据快照**：代码按分析范围强制生成固定采集计划；七项分析采集 17 类数据、24 个请求，三项速览采集 10 类数据、17 个请求。每项带请求编号、采集时间、耗时和“成功/正常无记录/失败/输入无效”状态，并随报告持久化。
- **代码结论一致性校验**：最终评级必须在五档范围内且唯一；若评级与明确操作方向冲突、评级缺失或关键数据缺失，系统停止展示操作结论，并禁止把该结果写入经验记忆。
- **快照状态展示**：报告页可查看本次计划采集数、可用数、失败数、快照编号和逐栏目状态。

### Changed

- **分析师只负责解释**：七位分析师不再绑定或调用数据工具，只能读取代码分配的快照子集；模型无法自行少取、补取或用常识填补失败栏目。
- **图编排先采后析**：流程固定为“统一采集数据 → 七位分析师并行分析 → 质量门控 → 决策链”，共享数据只采集一次，原分析师工具循环和运行内请求缓存退出运行路径。
- **用户结论由代码收口**：报告抬头与“一眼看懂”区域展示代码校验后的中文评级、操作倾向和风险提示；模型原始最终文本降为结论依据，不再直接充当产品操作指令。
- **旧断点自动隔离**：快照优先图与旧工具循环图不兼容，断点命名空间已升级；升级前未跑完的任务会从头重新分析，不会恢复到已删除的工具节点。已完成报告不受影响。

## [0.2.28] - 2026-07-18

### Added

- **历史记录可信度星级**：每份结果显示 0—5 星，星级同时考虑分析范围和数据可用性；七项分析且数据齐全为五星，三项速览即使范围内数据齐全也最多四星，关键数据缺失为一星并禁止给出操作结论。
- **三维状态展示**：流程状态、分析范围、数据状态分开显示为“已完成”“七项分析/三项速览”“数据齐全/部分数据获取成功/关键数据缺失”，网页抬头、历史列表、Markdown 和 PDF 使用同一套口径。

### Changed

- **应采未调用也进入门控**：质量门控按启用分析师汇总应采工具；模型完全没有调用的工具记为“未获取”，基础行情未调用直接限制为低可信，其他应采项未调用限制为中可信。正常空结果仍算接口成功，不降级。
- **消除“完整”歧义**：Web 不再用“完整”同时表达流程跑完和数据齐全，原模式名称改为“七项分析”，快速模式改为“三项速览”。
